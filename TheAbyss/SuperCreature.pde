...
abstract class SuperCreature {
  ...
  public float energy; // had to change this variable to public in order to let TAEegRepresentation live forever
  private float power;
  ...
  void applyTransforms() {
    translate(pos.x % width, pos.y % height, pos.z); // %width and %height to moreORless constrain creatures to screen
    rotateX(rot.x);
    rotateY(rot.y);
    rotateZ(rot.z);
  ...
  private void preDraw() {
    energy = max(0, energy - power); //creatures with eocnergy == 0 will be removed
    pushStyle();
